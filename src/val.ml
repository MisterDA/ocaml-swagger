  ...
  let to_sig { name; params; return; kind; descr } =
    ...
    let type_ =
      List.fold_right
        (fun param return ->
          match param with
          | Nolabel t -> ptyp_arrow Nolabel t return
          | Labelled ({ name; _ }, t) -> ptyp_arrow (Labelled name) t return
          | Optional ({ name; _ }, t) -> ptyp_arrow (Optional name) t return)
        params return
    in
...
module Impl = struct
  ...
  let assoc_string_with f params = params |> List.map f |> Ast_builder.elist

  let assoc_opt_string =
    let expr_of name = function
      | { Ast.ptyp_desc = Ptyp_constr (lident, _); _ } ->
          Ast_builder.type_constr_conv lident ~f:(sprintf "expr_of_%s")
            [ Ast_builder.evar name ]
      | _ -> assert false
    in
    let expr_opt_to_expr name =
      let name = Ast_builder.evar name in
      [%expr match [%e name] with Some s -> s | None -> ""]
    in
    let opt_to_expr name = function
      | { Ast.ptyp_desc = Ptyp_constr (lident, _); _ } ->
          let name = Ast_builder.evar name in
          let conv =
            Ast_builder.type_constr_conv lident ~f:(sprintf "expr_of_%s")
    ...
    assoc_string_with (fun p ->
        let orig_name, value =
          ...
          | Labelled ({ name; type_ }, Some origin) ->
              (origin.orig_name, expr_of name type_)
          | Labelled ({ name; type_ = [%type: string] }, None) ->
              (name, Ast_builder.evar name)
          | Labelled ({ name; type_ }, None) -> (name, expr_of name type_)
          | Optional ({ name; type_ = [%type: string] }, Some origin) ->
              (origin.orig_name, expr_opt_to_expr name)
          | Optional ({ name; type_ }, Some origin) ->
              (origin.orig_name, opt_to_expr name type_)
          | Optional ({ name; type_ }, None) -> (name, opt_to_expr name type_)
  ...
  let make_body params =
    ...
    | [ p ] ->
        let to_yojson =
          match param_type p with
          | { Ast.ptyp_desc = Ptyp_constr ({ txt = lident; _ }, _); _ } ->
              (Astlib.Longident.flatten lident
              |> Util.unsnoc |> Option.get |> snd)
              ^ ".to_yojson"
              |> Ast_builder.evar
          | _ -> assert false
        in
        ...
        [%expr
          Some
            (Body.of_string (Yojson.Safe.to_string ([%e to_yojson] [%e name])))]
  ...
  let make_response_code ?(body_param = false) ~return verb params =
    ...
    let return =
      match return with
      | Module module_name ->
          let of_yojson = Ast_builder.evar (module_name ^ ".of_yojson") in
          [%expr
            let json = Yojson.Safe.from_string body in
            Lwt.return
              (if code >= 200 && code < 300 then [%e of_yojson] json
              else Error body)]
      | Type type_name ->
          let conv_result =
            ...
            | { Ast.ptyp_desc = Ptyp_constr (lident, _); _ } ->
                Ast_builder.type_constr_conv lident ~f:(sprintf "%s_of_string")
                  [ [%expr body] ]
            | _ -> assert false
          in
          [%expr
  ...
  let build_http_request ?body_param ~return verb params =
    ...
    [%expr
      ...
      let uri =
        Uri.with_query' uri
          (List.filter (fun (_k, v) -> v <> {ocamlswagger||ocamlswagger}) query)
      in
  ...
  let make_fun params k =
    let to_pexp_fun k = function
      | Labelled (p, _) ->
          Ast_builder.(pexp_fun Ast.(Labelled p.name) None (pvar p.name) k)
      | Nolabel p -> Ast_builder.(pexp_fun Ast.Nolabel None (pvar p.name) k)
      | Optional (p, _) ->
          Ast_builder.(pexp_fun Ast.(Optional p.name) None (pvar p.name) k)
    in

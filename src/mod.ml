...
let rec to_module_type m =
  ...
  let items =
    ...
    |> List.rev_append values |> List.rev_append types
    |> List.rev_append submods |> Util.opt_cons definitions
  in
...
let rec to_module_expr m =
  let submods =
    m.submodules |> StringMap.bindings
    |> List.concat_map (fun (_name, m) -> to_module_expr m)
  in
  let items =
    submods
    @ List.concat_map
        (fun t -> Type.Impl.to_impl (Type.implementation t))
        m.types
    @ List.map (fun v -> Val.Impl.to_impl (Val.implementation v)) m.values
...
let to_mod m =
  ...
  let module_expr = to_module_expr m in
  let binding =
    module_binding ~name
      ~expr:(pmod_constraint (pmod_structure module_expr) module_type)
  in
